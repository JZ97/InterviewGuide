...
mov dword ptr [ebp-8],eax 这条语句将eax的值(即就是地址ebp-4)放入b的地址
...
实现多态有二种方式：覆蓋（override），重載（overload）。覆盖：是指子类重新定义父类的虚函数的做法。重载：是指允许存在多个同名函数，而这些函数的参数表不同（或许参数个数不同，或许参数类型不同，或许两者都不同）。例如：基类是一个抽象对象——人，那教师、运动员也是人，而使用这个抽象对象既可以表示教师、也可以表示运动员。
...
**(4) auto_ptr** （https://blog.csdn.net/czc1997/article/details/84026887）
...
#### 111、 继承机制中对象之间如何转换？指针和引用之间如何转换？(https://blog.csdn.net/weixin_44212574/article/details/89043854)
...
参考：https://blog.csdn.net/haoel/article/details/1948051/
 ...
 菱形继承的定义是：两个子类继承同一父类，而又有子类同时继承这两个子类。例如a,b两个类同时继承c，但是又有一个d类同时继承a,b类。(菱形继承解決：虛繼承)
...
#### 173、智能指针出现循环引用怎么解决？（https://www.pianshen.com/article/599096884/）
...
#### 180、C++从代码到可执行程序经历了什么？（https://www.runoob.com/w3cnote/cpp-header.html）
...
如果进线程无法取得锁，进线程不会立刻放弃CPU时间片，而是一直循环尝试获取锁，直到获取为止。如果别的线程长时期占有锁，那么自旋就是在浪费CPU做无用功，但是自旋锁一般应用于加锁时间很短的场景，这个时候效率比较高。https://www.cnblogs.com/FateTHarlaown/p/9170474.html
